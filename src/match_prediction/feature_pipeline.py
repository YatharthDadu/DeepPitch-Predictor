...
def build_historical_features(match_history: pd.DataFrame, rolling_window: int = 5) -> pd.DataFrame:
    ...
    try:
        ...
        home_cols = ['Date', 'Team', 'recent_goals_scored', 'recent_goals_conceded', 'recent_points',
                     'recent_goal_diff', 'recent_clean_sheets']
        home_features = team_history[home_cols].copy()
        home_features.columns = ['Date', 'HomeTeam', 'home_recent_goals_scored', 'home_recent_goals_conceded',
                                 'home_recent_points', 'home_recent_goal_diff', 'home_recent_clean_sheets']
        away_features = team_history[home_cols].copy()
        away_features.columns = ['Date', 'AwayTeam', 'away_recent_goals_scored', 'away_recent_goals_conceded',
                                 'away_recent_points', 'away_recent_goal_diff', 'away_recent_clean_sheets']
